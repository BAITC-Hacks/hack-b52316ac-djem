...
READ_TOOLS = [
    ...
    schema('calculate_forecast', 'Compute fact and forecast using the authoritative Python model. Use this tool for all numbers; never invent them.', {'plan': PLAN, 'dataset_version': I}, ['plan', 'dataset_version']),
...
WRITE_TOOLS = [
    ...
    schema('save_scenario', 'Create or update a database scenario. Updating requires its ID and current revision. A complete valid five-decision plan is required.', {'name': S, 'plan': PLAN, 'dataset_version': I, 'scenario_id': {'type': ['string', 'null']}, 'expected_revision': {'type': ['integer', 'null']}}, ['name', 'plan', 'dataset_version']),
...
def execute_tool(name, args, actor='agent', allow_write=True):
    ...
    if name == 'get_city_state':
        return {'dataset': db.get_dataset(), 'settings': db.get_settings(), 'scenarios': [{'id': s['id'], 'name': s['name'], 'revision': s['revision'], 'dataset_version': s['dataset_version'], 'plan': s['plan']} for s in db.list_scenarios()]}
    if name == 'calculate_forecast':
        dataset = db.get_dataset(args['dataset_version'])
        return forecast(dataset['data'], args['plan'])
    ...
    if name == 'save_scenario':
        return db.save_scenario(args['name'], args['plan'], args['dataset_version'], actor, args.get('scenario_id'), args.get('expected_revision'))
...
Все денежные суммы — тенге KZT. Данные синтетические, не официальная статистика и не реальные сметы.
